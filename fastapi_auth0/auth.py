...
from fastapi.security import (  # noqa: TC002
...
class Auth0UnauthenticatedException(HTTPException):
    def __init__(self, detail: str, **kwargs: int) -> None:
        """Returns HTTP 401"""
...
class Auth0:  # noqa: SIM119  # TODO(jw): make this a dataclass?
    def __init__(
        self,
        domain: str,
        api_audience: str,
    ...
    async def get_user(
        ...
        creds: Optional[HTTPAuthorizationCredentials] = Depends(Auth0HTTPBearer(auto_error=False)),  # noqa: B008
    ) -> Optional[Auth0User]:
        """
        Verify the Authorization: Bearer token and return the user.
        If there is any problem and auto_error = True then raise Auth0UnauthenticatedException or
        Auth0UnauthorizedException, otherwise return None.
        ...
        if creds is None:
            if self.auto_error:
                ...
                # 0.65.1 - https://github.com/tiangolo/fastapi/blob/aece74982d7c9c1acac98e2c872c4cb885677fc7/fastapi/security/http.py  # noqa: E501
                raise HTTPException(
        ...
        try:
            ...
            # This is an unlikely case but handle it just to be safe (maybe the
            # token is specially crafted to bug our code)
